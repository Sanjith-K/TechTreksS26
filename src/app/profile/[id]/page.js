...
export default function ProfilePage() {
    ...
    useEffect(() => {
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/profiles/${id}`)
            .then((res) => {
    ...
    return (
        <main className="relative flex min-h-screen flex-col overflow-hidden bg-[#07152b] text-white">
            ...
            <div className="relative z-10 mx-auto w-full max-w-7xl flex-1 px-8 pb-28 pt-6">
                ...
                {loading && (
                    <p className="mt-12 text-center text-white/50">Loading profile...</p>
                )}

                {error && (
                    <p className="mt-12 text-center text-white/50">{error}</p>
                )}
                        ...
                        <section className="mt-6 overflow-hidden rounded-3xl border border-white/8 bg-white/8 backdrop-blur-md">
                            <div className="rounded-b-3xl bg-[#0c1b37] px-6 py-8">
                                <div className="flex flex-col items-center">
                                    ...
                                    <h1 className="mt-4 text-3xl font-semibold">{profile.name || "Unknown"}</h1>
                                    <p className="mt-1 text-white/55">{profile.university || "New York University"}</p>
                                ...
                                <div className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-3">
                                    <div className="rounded-2xl bg-white/5 p-4 text-center">
                                        <p className="text-2xl font-semibold">{profile.favorites_count ?? 0}</p>
                                        <p className="mt-1 text-sm text-white/45">Favorites</p>
                                    </div>
                                    <div className="rounded-2xl bg-white/5 p-4 text-center">
                                        <p className="text-2xl font-semibold">{profile.visited_count ?? 0}</p>
                                        <p className="mt-1 text-sm text-white/45">Visited</p>
                                    </div>
                                    <div className="rounded-2xl bg-white/5 p-4 text-center">
                                        <p className="text-2xl font-semibold">{profile.reviews_count ?? 0}</p>
                        ...
                        <section className="mt-8">
                            ...
                            <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
                                <div className="rounded-2xl border border-white/8 bg-white/8 p-4 backdrop-blur-md">
                                    <p className="text-3xl font-semibold">{profile.study_sessions ?? 0}</p>
                                    <p className="mt-1 text-sm text-white/45">Study Sessions</p>
                                </div>
                                <div className="rounded-2xl border border-white/8 bg-white/8 p-4 backdrop-blur-md">
                                    <p className="text-3xl font-semibold">{profile.hours_logged ?? 0}</p>
                                    <p className="mt-1 text-sm text-white/45">Hours Logged</p>
                                </div>
                                <div className="rounded-2xl border border-white/8 bg-white/8 p-4 backdrop-blur-md">
                                    <p className="text-3xl font-semibold">{profile.places_visited ?? 0}</p>
                                    <p className="mt-1 text-sm text-white/45">Places Visited</p>
                                </div>
                                <div className="rounded-2xl border border-white/8 bg-white/8 p-4 backdrop-blur-md">
                                    <p className="text-3xl font-semibold">{profile.reviews_count ?? 0}</p>
                                    <p className="mt-1 text-sm text-white/45">Reviews</p>
                                </div>
                            </div>
                        </section>
